...
export default function ConnectLinks({ latestVideo }: Props) {
  return (
    <Card className="p-6 shadow-lg bg-[#2C3A45]">
      ...
      <div className="grid grid-cols-1 gap-4">
        <div className="p-4 rounded-lg shadow bg-[#3E4E5A]">
          ...
          <a href={latestVideo?.url || "https://www.youtube.com/@krr"} target="_blank" rel="noopener noreferrer">
            <div className="aspect-video mb-2 relative">
              <Image
                src={latestVideo?.thumbnailUrl || "/images/YouTube.jpeg"}
                alt={latestVideo?.title || "YouTubeチャンネルサムネイル"}
                fill
                className="object-cover rounded"
                sizes="(max-width: 768px) 100vw, 33vw"
                priority={!!latestVideo}
              />
            </div>
          </a>
